...
parser.add_argument('--data_path', default='improve/tmp/mendota/train/similar_980_1/inputs')

# parser.add_argument('--restore_path', default='improve/tmp/mendo/model1')
parser.add_argument('--save_path', default='improve/tmp/mendota/model')
parser.add_argument('--preds_path', default='improve/tmp/mendota/out')
...
def den_loss(temp):
    ...
    den_d1 = den[0:n_depths-1,:]
    den_d2 = den[1:n_depths, :]
...
with tf.variable_scope("rnn2", reuse=True) as scope_sp1:
    state_series_xu_d, current_state_xu_d = tf.nn.dynamic_rnn(lstm_cell_depth, un_x_d, dtype=tf.float32, scope=scope_sp)
...
def iter(ii):
    ...
    train_data = feather.read_dataframe(os.path.join(args.data_path, 'labels_train'+str(ii)+'.feather'))
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(epochs):
            ...
            if (epoch+1)%100 == 0:
                ...
                np.savetxt(os.path.join(args.preds_path, "predict_model2_+_"+str(ii)+"_"+str(epoch+1)+".csv"), prd_o, delimiter=',')
